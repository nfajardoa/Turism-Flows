...
quietly xtreg ${xtendog} l_pib l_dist l_itcrbinv l_spex l_comercio_usd_2010_resid ll_giro_rem conflicto y2002-y2017 if ${s_year}<=year & year<=${f_year} & pais_num<=${n_countries} , fe
estimates store within_total
xthtaylor ${xtendog} l_pib l_dist l_itcrbinv l_spex l_comercio_usd_2010_resid ll_giro_rem conflicto y2002-y2017 if ${s_year}<=year & year<=${f_year} & pais_num<=${n_countries} , endog(l_pib l_itcrbinv l_spex) constant(l_dist) small
hausman within_total .

quietly xtreg ${xtendog} l_pib l_dist l_itcrbinv l_spex l_comercio_usd_2010_resid ll_giro_rem conflicto ll_sillas y2002-y2017 if ${s_year}<=year & year<=${f_year} & pais_num<=${n_countries} , fe
estimates store within_total
quietly xthtaylor ${xtendog} l_pib l_dist l_itcrbinv l_spex l_comercio_usd_2010_resid ll_giro_rem conflicto ll_sillas y2002-y2017 if ${s_year}<=year & year<=${f_year} & pais_num<=${n_countries} , endog(l_pib l_itcrbinv l_spex) constant(l_dist) small
hausman within_total .

quietly xtreg ${xtendog} l_pib l_dist l_itcrbinv l_spex l_comercio_usd_2010_resid ll_giro_rem conflicto ll_sillas l_habs_ph_1 y2002-y2017 if ${s_year}<=year & year<=${f_year} & pais_num<=${n_countries} , fe 
estimates store within_total
quietly xthtaylor ${xtendog} l_pib l_dist l_itcrbinv l_spex l_comercio_usd_2010_resid ll_giro_rem conflicto ll_sillas l_habs_ph_1 y2002-y2017 if ${s_year}<=year & year<=${f_year} & pais_num<=${n_countries} , endog(l_pib l_itcrbinv l_spex) constant(l_dist) small
hausman within_total .

global xtendog "l_turis_col"

quietly xtreg ${xtendog} l_pib l_dist l_itcrbinv l_spex l_comercio_usd_2010_resid ll_giro_rem conflicto y2002-y2017 if ${s_year}<=year & year<=${f_year} & pais_num<=${n_countries} , fe
estimates store within_col
quietly xthtaylor ${xtendog} l_pib l_dist l_itcrbinv l_spex l_comercio_usd_2010_resid ll_giro_rem conflicto y2002-y2017 if ${s_year}<=year & year<=${f_year} & pais_num<=${n_countries} , endog(l_pib l_itcrbinv l_spex) constant(l_dist)  small
hausman within_col .
