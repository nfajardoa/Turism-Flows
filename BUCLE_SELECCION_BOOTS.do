...
xthtaylor ${xtendog} `x' if 2000<=year & year<=${f_year} /*& pais_num < 20*/, endog(${xtendog}) constant(${xtconst})  small  vce(boots, reps(500))
outreg2 using Estimates_small_20paises_bootstrap_${xtendog}_${f_year}_06-06-2018.xlsx, excel append label sortvar(l_pib l_dist l_itcr l_itcrbinv lspex l${xtendog} l_comercio_usd_2010 ll_giro_rem conflicto l_habs_1 l_sillas) dec(2) 
