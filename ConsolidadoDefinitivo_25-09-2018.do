...
xthtaylor ${xtendog} l_pib l_dist l_itcr espanol tlc y2002-y2017 if ${s_year}<=year & year<=${f_year} & pais_num<=${n_countries} , endog(${xtendog}) constant(l_dist espanol) small vce(robust)
...
xthtaylor ${xtendog} l_pib l_dist y2002-y2017 if ${s_year}<=year & year<=${f_year} & pais_num<=${n_countries} , endog(${xtendog}) constant(l_dist)  small vce(robust)
outreg2 using Estimates_Small_2etapas_${s_year}-${f_year}_${date}_total_definitivo.xls, excel append label sortvar(l_pib l_dist l_itcrbinv l_spex l_comercio_usd_2010_resid ll_giro_rem conflicto ll_sillas l_habs_ph_1 espanol) dec(2)

xthtaylor ${xtendog} l_pib espanol y2002-y2017 if ${s_year}<=year & year<=${f_year} & pais_num<=${n_countries} , endog(${xtendog}) constant(espanol)  small vce(robust)
outreg2 using Estimates_Small_2etapas_${s_year}-${f_year}_${date}_total_definitivo.xls, excel append label sortvar(l_pib l_dist l_itcrbinv l_spex l_comercio_usd_2010_resid ll_giro_rem conflicto ll_sillas l_habs_ph_1 espanol) dec(2)

xthtaylor ${xtendog} l_pib l_dist espanol y2002-y2017 if ${s_year}<=year & year<=${f_year} & pais_num<=${n_countries} , endog(${xtendog}) constant(l_dist espanol)  small vce(robust)
outreg2 using Estimates_Small_2etapas_${s_year}-${f_year}_${date}_total_definitivo.xls, excel append label sortvar(l_pib l_dist l_itcrbinv l_spex l_comercio_usd_2010_resid ll_giro_rem conflicto ll_sillas l_habs_ph_1 espanol) dec(2)

oneway l_dist espanol if ${s_year}<=year & year<=${f_year} & pais_num<=${n_countries}, tabulate scheffe

spearman l_dist espanol if ${s_year}<=year & year<=${f_year} & pais_num<=${n_countries}

ktau l_dist espanol if ${s_year}<=year & year<=${f_year} & pais_num<=${n_countries}

corr l_dist espanol if ${s_year}<=year & year<=${f_year} & pais_num<=${n_countries}

kwallis l_dist, by(espanol)

*SIN DISTANCIA NI ESPAÑOL
global xtmodels ""l_pib" "l_pib l_itcrbinv l_spex l_comercio_usd_2010_resid ll_giro_rem conflicto" "l_pib l_itcrbinv l_spex l_comercio_usd_2010_resid ll_giro_rem conflicto ll_sillas" "l_pib l_itcrbinv l_spex l_comercio_usd_2010_resid ll_giro_rem conflicto ll_sillas l_habs_ph_1""

foreach x of global xtmodels{

xtreg ${xtendog} `x' y2002-y2017 if ${s_year}<=year & year<=${f_year} & pais_num<=${n_countries}, fe vce(robust)
outreg2 using Estimates_Small_2etapas_${s_year}-${f_year}_${date}_total_definitivo.xls, excel append label sortvar(l_pib l_dist l_itcrbinv l_spex l_comercio_usd_2010_resid ll_giro_rem conflicto ll_sillas l_habs_ph_1 espanol) dec(2) 
...
xthtaylor ${xtendog} `x' y2002-y2017 if ${s_year}<=year & year<=${f_year} & pais_num<=${n_countries} , endog(${xtendog}) constant(${xtconst})  small vce(robust)
...
xthtaylor ${xtendog} l_pib l_dist l_itcr espanol tlc y2002-y2017 if ${s_year}<=year & year<=${f_year} & pais_num<=${n_countries} , endog(${xtendog}) constant(l_dist espanol) vce(robust)
predict ${xtendog}_resid, ue
label variable  ${xtendog}_resid         "ln Comercio en dolares ctes de 2010 (residuos)"
outreg2 using Estimates_small_${xtendog}_${s_year}-${f_year}_${date}.xls, excel append label sortvar(l_pib l_dist l_itcr l_itcrbinv lspex l${xtendog} l_comercio_usd_2010 ll_giro_rem conflicto l_habs_1 ll_sillas tlc espanol y2002-y2017) dec(2) 
