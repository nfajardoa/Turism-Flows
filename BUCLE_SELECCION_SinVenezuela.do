...
outreg2 using Estimates_small_19paises_${xtendog}_${s_year}_${f_year}_07-06-2018.xls, excel append label sortvar(l_pib l_dist l_itcr l_itcrbinv lspex l${xtendog} l_comercio_usd_2010 ll_giro_rem conflicto l_habs_1 ll_sillas) dec(2) 
...
xthtaylor ${xtendog} `x' if ${s_year}<=year & year<=${f_year} & pais_num < 20, endog(${xtendog}) constant(${xtconst})  small 
outreg2 using Estimates_small_19paises_${xtendog}_${s_year}_${f_year}_07-06-2018.xls, excel append label sortvar(l_pib l_dist l_itcr l_itcrbinv lspex l${xtendog} l_comercio_usd_2010 ll_giro_rem conflicto l_habs_1 l_sillas) dec(2) 
replace count 		= count + 1
global counter 		= count
}
